# <a name="variables"></a><span data-ttu-id="3c9c4-101">Переменные</span><span class="sxs-lookup"><span data-stu-id="3c9c4-101">Variables</span></span>
...
<span data-ttu-id="3c9c4-126">Для целей проверки определенного присваивания переменной экземпляра класса считается начальным значением.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-126">For the purpose of definite assignment checking, an instance variable of a class is considered initially assigned.</span></span>
...
<span data-ttu-id="3c9c4-151">Внутри метода экземпляра или методе доступа экземпляра типа структуры `this` ключевое слово ведет себя точно так, как ссылочный параметр типа структуры ([такой доступ](expressions.md#this-access)).</span><span class="sxs-lookup"><span data-stu-id="3c9c4-151">Within an instance method or instance accessor of a struct type, the `this` keyword behaves exactly as a reference parameter of the struct type ([This access](expressions.md#this-access)).</span></span>
...
*  <span data-ttu-id="3c9c4-226">Переменные экземпляра структуры изначально не присвоены значения переменных.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-226">Instance variables of initially unassigned struct variables.</span></span>
...
#### <a name="block-statements-checked-and-unchecked-statements"></a><span data-ttu-id="3c9c4-258">Блок операторов, этот флажок установлен и снят флажок операторов</span><span class="sxs-lookup"><span data-stu-id="3c9c4-258">Block statements, checked, and unchecked statements</span></span>
...
<span data-ttu-id="3c9c4-268">Для `if` инструкции *stmt* формы:</span><span class="sxs-lookup"><span data-stu-id="3c9c4-268">For an `if` statement *stmt* of the form:</span></span>
```csharp
...
#### <a name="while-statements"></a><span data-ttu-id="3c9c4-279">While-операторы</span><span class="sxs-lookup"><span data-stu-id="3c9c4-279">While statements</span></span>
...
#### <a name="for-statements"></a><span data-ttu-id="3c9c4-291">Для инструкций</span><span class="sxs-lookup"><span data-stu-id="3c9c4-291">For statements</span></span>
...
#### <a name="return-statements"></a><span data-ttu-id="3c9c4-300">Операторы "Return"</span><span class="sxs-lookup"><span data-stu-id="3c9c4-300">Return statements</span></span>
...
<span data-ttu-id="3c9c4-325">Анализ определенного присваивания для `try` - `catch` - `finally` инструкцию для формы:</span><span class="sxs-lookup"><span data-stu-id="3c9c4-325">Definite assignment analysis for a `try`-`catch`-`finally` statement of the form:</span></span>
```csharp
try try_block
catch(...) catch_block_1
...
...
#### <a name="foreach-statements"></a><span data-ttu-id="3c9c4-328">Инструкции по каждому элементу</span><span class="sxs-lookup"><span data-stu-id="3c9c4-328">Foreach statements</span></span>
...
#### <a name="yield-statements"></a><span data-ttu-id="3c9c4-340">Операторы yield</span><span class="sxs-lookup"><span data-stu-id="3c9c4-340">Yield statements</span></span>
...
#### <a name="invocation-expressions-and-object-creation-expressions"></a><span data-ttu-id="3c9c4-357">Выражения вызова и выражения создания объекта</span><span class="sxs-lookup"><span data-stu-id="3c9c4-357">Invocation expressions and object creation expressions</span></span>
...
#### <a name="simple-assignment-expressions"></a><span data-ttu-id="3c9c4-368">Простые выражения присваивания</span><span class="sxs-lookup"><span data-stu-id="3c9c4-368">Simple assignment expressions</span></span>
...
*  <span data-ttu-id="3c9c4-370">Состояние определенного присваивания *v* перед *expr_rhs* совпадает со значением состояния определенного присваивания *v* перед *expr*.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-370">The definite assignment state of *v* before *expr_rhs* is the same as the definite assignment state of *v* before *expr*.</span></span>
*  <span data-ttu-id="3c9c4-371">Состояние определенного присваивания *v* после *expr* определяется:</span><span class="sxs-lookup"><span data-stu-id="3c9c4-371">The definite assignment state of *v* after *expr* is determined by:</span></span>
   * <span data-ttu-id="3c9c4-372">Если *w* — это та же переменная, как *v*, состояние определенного присваивания переменной *v* после *expr* присваивается определенным образом.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-372">If *w* is the same variable as *v*, then the definite assignment state of *v* after *expr* is definitely assigned.</span></span>
   * <span data-ttu-id="3c9c4-373">В противном случае, если назначение происходит в конструкторе экземпляра типа структуры, если *w* имеет доступ к свойству, обозначающий автоматически реализуемого свойства *P* конструируемого экземпляра и *v* является скрытым резервное поле *P*, состояние определенного присваивания переменной *v* после *expr* определенно назначить.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-373">Otherwise, if the assignment occurs within the instance constructor of a struct type, if *w* is a property access designating an automatically implemented property *P* on the instance being constructed and *v* is the hidden backing field of *P*, then the definite assignment state of *v* after *expr* is definitely assigned.</span></span>
   * <span data-ttu-id="3c9c4-374">В противном случае состояние определенного присваивания *v* после *expr* совпадает со значением состояния определенного присваивания *v* после *expr_rhs*.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-374">Otherwise, the definite assignment state of *v* after *expr* is the same as the definite assignment state of *v* after *expr_rhs*.</span></span>

#### <a name="-conditional-and-expressions"></a><span data-ttu-id="3c9c4-375">& & (условный оператор AND) выражения</span><span class="sxs-lookup"><span data-stu-id="3c9c4-375">&& (conditional AND) expressions</span></span>

<span data-ttu-id="3c9c4-376">Для выражения *expr* формы `expr_first && expr_second`:</span><span class="sxs-lookup"><span data-stu-id="3c9c4-376">For an expression *expr* of the form `expr_first && expr_second`:</span></span>

*  <span data-ttu-id="3c9c4-377">Состояние определенного присваивания *v* перед *expr_first* совпадает со значением состояния определенного присваивания *v* перед *expr*.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-377">The definite assignment state of *v* before *expr_first* is the same as the definite assignment state of *v* before *expr*.</span></span>
*  <span data-ttu-id="3c9c4-378">Состояние определенного присваивания *v* перед *expr_second* определенно присвоенной, если состояние *v* после *expr_first* либо определенно присвоенной или «определенно присвоенной после выражения true».</span><span class="sxs-lookup"><span data-stu-id="3c9c4-378">The definite assignment state of *v* before *expr_second* is definitely assigned if the state of *v* after *expr_first* is either definitely assigned or "definitely assigned after true expression".</span></span> <span data-ttu-id="3c9c4-379">В противном случае он не назначается явно.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-379">Otherwise, it is not definitely assigned.</span></span>
*  <span data-ttu-id="3c9c4-380">Состояние определенного присваивания *v* после *expr* определяется:</span><span class="sxs-lookup"><span data-stu-id="3c9c4-380">The definite assignment state of *v* after *expr* is determined by:</span></span>
    * <span data-ttu-id="3c9c4-381">Если *expr_first* является константным выражением со значением `false`, состояние определенного присваивания переменной *v* после *expr* совпадает со значением определенного присваивания состояние *v* после *expr_first*.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-381">If *expr_first* is a constant expression with the value `false`, then the definite assignment state of *v* after *expr* is the same as the definite assignment state of *v* after *expr_first*.</span></span>
    * <span data-ttu-id="3c9c4-382">В противном случае, если состояние *v* после *expr_first* является определенно присвоенной, то состояние *v* после *expr* присваивается определенным образом.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-382">Otherwise, if the state of *v* after *expr_first* is definitely assigned, then the state of *v* after *expr* is definitely assigned.</span></span>
    * <span data-ttu-id="3c9c4-383">В противном случае, если состояние *v* после *expr_second* определенно присвоенной и состояние *v* после *expr_first* определенно « назначенные после выражения false», а затем состояние *v* после *expr* присваивается определенным образом.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-383">Otherwise, if the state of *v* after *expr_second* is definitely assigned, and the state of *v* after *expr_first* is "definitely assigned after false expression", then the state of *v* after *expr* is definitely assigned.</span></span>
    * <span data-ttu-id="3c9c4-384">В противном случае, если состояние *v* после *expr_second* определенно присвоенной или «определенно присвоенной после значение true, выражение», а затем состояние *v* после  *expr* «присваивается определенным образом после выражения true».</span><span class="sxs-lookup"><span data-stu-id="3c9c4-384">Otherwise, if the state of *v* after *expr_second* is definitely assigned or "definitely assigned after true expression", then the state of *v* after *expr* is "definitely assigned after true expression".</span></span>
    * <span data-ttu-id="3c9c4-385">В противном случае, если состояние *v* после *expr_first* «определенно присвоенной после выражения false», а состояние *v* после *expr_second* «определенно присвоенной после выражения false», то состояние *v* после *expr* «присваивается определенным образом после выражения false».</span><span class="sxs-lookup"><span data-stu-id="3c9c4-385">Otherwise, if the state of *v* after *expr_first* is "definitely assigned after false expression", and the state of *v* after *expr_second* is "definitely assigned after false expression", then the state of *v* after *expr* is "definitely assigned after false expression".</span></span>
    * <span data-ttu-id="3c9c4-386">В противном случае состояние *v* после *expr* определенно не назначается.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-386">Otherwise, the state of *v* after *expr* is not definitely assigned.</span></span>

<span data-ttu-id="3c9c4-387">В примере</span><span class="sxs-lookup"><span data-stu-id="3c9c4-387">In the example</span></span>
```csharp
class A
{
...
```
<span data-ttu-id="3c9c4-388">переменная `i` считается определенно присвоенной в одном из внедренных операторов `if` инструкции, но не в другой.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-388">the variable `i` is considered definitely assigned in one of the embedded statements of an `if` statement but not in the other.</span></span> <span data-ttu-id="3c9c4-389">В `if` инструкция в методе `F`, переменная `i` определенно присвоенной первый внедренный оператор, поскольку выполнение выражения `(i = y)` всегда предшествует выполнению этого внедренного оператора.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-389">In the `if` statement in method `F`, the variable `i` is definitely assigned in the first embedded statement because execution of the expression `(i = y)` always precedes execution of this embedded statement.</span></span> <span data-ttu-id="3c9c4-390">Напротив, переменная `i` не является определенно присвоенной во втором операторе внедренных, так как `x >= 0` может завершиться с итогом false, приводит к переменной `i` не назначены.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-390">In contrast, the variable `i` is not definitely assigned in the second embedded statement, since `x >= 0` might have tested false, resulting in the variable `i` being unassigned.</span></span>

#### <a name="-conditional-or-expressions"></a><span data-ttu-id="3c9c4-391">|| (условный оператор OR) выражения</span><span class="sxs-lookup"><span data-stu-id="3c9c4-391">|| (conditional OR) expressions</span></span>

<span data-ttu-id="3c9c4-392">Для выражения *expr* формы `expr_first || expr_second`:</span><span class="sxs-lookup"><span data-stu-id="3c9c4-392">For an expression *expr* of the form `expr_first || expr_second`:</span></span>

*  <span data-ttu-id="3c9c4-393">Состояние определенного присваивания *v* перед *expr_first* совпадает со значением состояния определенного присваивания *v* перед *expr*.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-393">The definite assignment state of *v* before *expr_first* is the same as the definite assignment state of *v* before *expr*.</span></span>
*  <span data-ttu-id="3c9c4-394">Состояние определенного присваивания *v* перед *expr_second* определенно присвоенной, если состояние *v* после *expr_first* либо определенно присвоенной или «определенно присвоенной после выражения false».</span><span class="sxs-lookup"><span data-stu-id="3c9c4-394">The definite assignment state of *v* before *expr_second* is definitely assigned if the state of *v* after *expr_first* is either definitely assigned or "definitely assigned after false expression".</span></span> <span data-ttu-id="3c9c4-395">В противном случае он не назначается явно.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-395">Otherwise, it is not definitely assigned.</span></span>
*  <span data-ttu-id="3c9c4-396">Определенного присваивания переменной *v* после *expr* определяется:</span><span class="sxs-lookup"><span data-stu-id="3c9c4-396">The definite assignment statement of *v* after *expr* is determined by:</span></span>
    * <span data-ttu-id="3c9c4-397">Если *expr_first* является константным выражением со значением `true`, состояние определенного присваивания переменной *v* после *expr* совпадает со значением определенного присваивания состояние *v* после *expr_first*.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-397">If *expr_first* is a constant expression with the value `true`, then the definite assignment state of *v* after *expr* is the same as the definite assignment state of *v* after *expr_first*.</span></span>
    * <span data-ttu-id="3c9c4-398">В противном случае, если состояние *v* после *expr_first* является определенно присвоенной, то состояние *v* после *expr* присваивается определенным образом.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-398">Otherwise, if the state of *v* after *expr_first* is definitely assigned, then the state of *v* after *expr* is definitely assigned.</span></span>
    * <span data-ttu-id="3c9c4-399">В противном случае, если состояние *v* после *expr_second* определенно присвоенной и состояние *v* после *expr_first* определенно « назначенные после выражения true», а затем состояние *v* после *expr* присваивается определенным образом.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-399">Otherwise, if the state of *v* after *expr_second* is definitely assigned, and the state of *v* after *expr_first* is "definitely assigned after true expression", then the state of *v* after *expr* is definitely assigned.</span></span>
    * <span data-ttu-id="3c9c4-400">В противном случае, если состояние *v* после *expr_second* определенно присвоенной или «определенно присвоенной после выражения false», а затем состояние *v* после *expr* «присваивается определенным образом после выражения false».</span><span class="sxs-lookup"><span data-stu-id="3c9c4-400">Otherwise, if the state of *v* after *expr_second* is definitely assigned or "definitely assigned after false expression", then the state of *v* after *expr* is "definitely assigned after false expression".</span></span>
    * <span data-ttu-id="3c9c4-401">В противном случае, если состояние *v* после *expr_first* «определенно присвоенной после выражения true», а состояние *v* после *expr_second*«определенно присвоенной после выражения true», то состояние *v* после *expr* «присваивается определенным образом после выражения true».</span><span class="sxs-lookup"><span data-stu-id="3c9c4-401">Otherwise, if the state of *v* after *expr_first* is "definitely assigned after true expression", and the state of *v* after *expr_second* is "definitely assigned after true expression", then the state of *v* after *expr* is "definitely assigned after true expression".</span></span>
    * <span data-ttu-id="3c9c4-402">В противном случае состояние *v* после *expr* определенно не назначается.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-402">Otherwise, the state of *v* after *expr* is not definitely assigned.</span></span>

<span data-ttu-id="3c9c4-403">В примере</span><span class="sxs-lookup"><span data-stu-id="3c9c4-403">In the example</span></span>
```csharp
class A
{
...
```
<span data-ttu-id="3c9c4-404">переменная `i` считается определенно присвоенной в одном из внедренных операторов `if` инструкции, но не в другой.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-404">the variable `i` is considered definitely assigned in one of the embedded statements of an `if` statement but not in the other.</span></span> <span data-ttu-id="3c9c4-405">В `if` инструкция в методе `G`, переменная `i` определенно присвоенной второй внедренный оператор, поскольку выполнение выражения `(i = y)` всегда предшествует выполнению этого внедренного оператора.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-405">In the `if` statement in method `G`, the variable `i` is definitely assigned in the second embedded statement because execution of the expression `(i = y)` always precedes execution of this embedded statement.</span></span> <span data-ttu-id="3c9c4-406">Напротив, переменная `i` не является определенно присвоенной в первом внедренный оператор, поскольку `x >= 0` может завершиться с итогом значение true, приводит к переменной `i` не назначены.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-406">In contrast, the variable `i` is not definitely assigned in the first embedded statement, since `x >= 0` might have tested true, resulting in the variable `i` being unassigned.</span></span>

#### <a name="-logical-negation-expressions"></a><span data-ttu-id="3c9c4-407">!</span><span class="sxs-lookup"><span data-stu-id="3c9c4-407">!</span></span> <span data-ttu-id="3c9c4-408">выражения (логическое отрицание)</span><span class="sxs-lookup"><span data-stu-id="3c9c4-408">(logical negation) expressions</span></span>

<span data-ttu-id="3c9c4-409">Для выражения *expr* формы `! expr_operand`:</span><span class="sxs-lookup"><span data-stu-id="3c9c4-409">For an expression *expr* of the form `! expr_operand`:</span></span>

*  <span data-ttu-id="3c9c4-410">Состояние определенного присваивания *v* перед *expr_operand* совпадает со значением состояния определенного присваивания *v* перед *expr*.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-410">The definite assignment state of *v* before *expr_operand* is the same as the definite assignment state of *v* before *expr*.</span></span>
*  <span data-ttu-id="3c9c4-411">Состояние определенного присваивания *v* после *expr* определяется:</span><span class="sxs-lookup"><span data-stu-id="3c9c4-411">The definite assignment state of *v* after *expr* is determined by:</span></span>
    * <span data-ttu-id="3c9c4-412">Если состояние *v* после \* expr_operand \* является определенно присвоенной, то состояние *v* после *expr* присваивается определенным образом.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-412">If the state of *v* after \*expr_operand \*is definitely assigned, then the state of *v* after *expr* is definitely assigned.</span></span>
    * <span data-ttu-id="3c9c4-413">Если состояние *v* после \* expr_operand \* не является определенно присвоенной, то состояние *v* после *expr* определенно не назначается.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-413">If the state of *v* after \*expr_operand \*is not definitely assigned, then the state of *v* after *expr* is not definitely assigned.</span></span>
    * <span data-ttu-id="3c9c4-414">Если состояние *v* после \* expr_operand \* «определенно присвоенной после выражения false», то состояние *v* после *expr* «присваивается определенным образом после true выражение».</span><span class="sxs-lookup"><span data-stu-id="3c9c4-414">If the state of *v* after \*expr_operand \*is "definitely assigned after false expression", then the state of *v* after *expr* is "definitely assigned after true expression".</span></span>
    * <span data-ttu-id="3c9c4-415">Если состояние *v* после \* expr_operand \* «определенно присвоенной после выражения true», то состояние *v* после *expr* «присваивается определенным образом после false выражение».</span><span class="sxs-lookup"><span data-stu-id="3c9c4-415">If the state of *v* after \*expr_operand \*is "definitely assigned after true expression", then the state of *v* after *expr* is "definitely assigned after false expression".</span></span>

#### <a name="-null-coalescing-expressions"></a><span data-ttu-id="3c9c4-416">??</span><span class="sxs-lookup"><span data-stu-id="3c9c4-416">??</span></span> <span data-ttu-id="3c9c4-417">выражения (нулем)</span><span class="sxs-lookup"><span data-stu-id="3c9c4-417">(null coalescing) expressions</span></span>

<span data-ttu-id="3c9c4-418">Для выражения *expr* формы `expr_first ?? expr_second`:</span><span class="sxs-lookup"><span data-stu-id="3c9c4-418">For an expression *expr* of the form `expr_first ?? expr_second`:</span></span>

*  <span data-ttu-id="3c9c4-419">Состояние определенного присваивания *v* перед *expr_first* совпадает со значением состояния определенного присваивания *v* перед *expr*.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-419">The definite assignment state of *v* before *expr_first* is the same as the definite assignment state of *v* before *expr*.</span></span>
*  <span data-ttu-id="3c9c4-420">Состояние определенного присваивания *v* перед *expr_second* совпадает со значением состояния определенного присваивания *v* после *expr_first*.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-420">The definite assignment state of *v* before *expr_second* is the same as the definite assignment state of *v* after *expr_first*.</span></span>
*  <span data-ttu-id="3c9c4-421">Определенного присваивания переменной *v* после *expr* определяется:</span><span class="sxs-lookup"><span data-stu-id="3c9c4-421">The definite assignment statement of *v* after *expr* is determined by:</span></span>
    * <span data-ttu-id="3c9c4-422">Если *expr_first* является константным выражением ([константные выражения](expressions.md#constant-expressions)) со значением null, то состояние *v* после *expr* совпадает состоянию *v* после *expr_second*.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-422">If *expr_first* is a constant expression ([Constant expressions](expressions.md#constant-expressions)) with value null, then the the state of *v* after *expr* is the same as the state of *v* after *expr_second*.</span></span>
*  <span data-ttu-id="3c9c4-423">В противном случае состояние *v* после *expr* совпадает со значением состояния определенного присваивания *v* после *expr_first*.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-423">Otherwise, the state of *v* after *expr* is the same as the definite assignment state of *v* after *expr_first*.</span></span>

#### <a name="-conditional-expressions"></a><span data-ttu-id="3c9c4-424">?: (условный) выражения</span><span class="sxs-lookup"><span data-stu-id="3c9c4-424">?: (conditional) expressions</span></span>

<span data-ttu-id="3c9c4-425">Для выражения *expr* формы `expr_cond ? expr_true : expr_false`:</span><span class="sxs-lookup"><span data-stu-id="3c9c4-425">For an expression *expr* of the form `expr_cond ? expr_true : expr_false`:</span></span>

*  <span data-ttu-id="3c9c4-426">Состояние определенного присваивания *v* перед *expr_cond* совпадает со значением состояния *v* перед *expr*.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-426">The definite assignment state of *v* before *expr_cond* is the same as the state of *v* before *expr*.</span></span>
*  <span data-ttu-id="3c9c4-427">Состояние определенного присваивания *v* перед *expr_true* является определенно присвоенной только в том случае, если содержит одно из следующих:</span><span class="sxs-lookup"><span data-stu-id="3c9c4-427">The definite assignment state of *v* before *expr_true* is definitely assigned if and only if one of the following holds:</span></span>
    * <span data-ttu-id="3c9c4-428">*expr_cond* является константным выражением со значением `false`</span><span class="sxs-lookup"><span data-stu-id="3c9c4-428">*expr_cond* is a constant expression with the value `false`</span></span>
    * <span data-ttu-id="3c9c4-429">состояние *v* после *expr_cond* имеет определенно присвоенной или «определенно назначены после выражения true».</span><span class="sxs-lookup"><span data-stu-id="3c9c4-429">the state of *v* after *expr_cond* is definitely assigned or "definitely assigned after true expression".</span></span>
*  <span data-ttu-id="3c9c4-430">Состояние определенного присваивания *v* перед *expr_false* является определенно присвоенной только в том случае, если содержит одно из следующих:</span><span class="sxs-lookup"><span data-stu-id="3c9c4-430">The definite assignment state of *v* before *expr_false* is definitely assigned if and only if one of the following holds:</span></span>
    * <span data-ttu-id="3c9c4-431">*expr_cond* является константным выражением со значением `true`</span><span class="sxs-lookup"><span data-stu-id="3c9c4-431">*expr_cond* is a constant expression with the value `true`</span></span>
*  <span data-ttu-id="3c9c4-432">состояние *v* после *expr_cond* имеет определенно присвоенной или «определенно присвоенной после выражения false».</span><span class="sxs-lookup"><span data-stu-id="3c9c4-432">the state of *v* after *expr_cond* is definitely assigned or "definitely assigned after false expression".</span></span>
*  <span data-ttu-id="3c9c4-433">Состояние определенного присваивания *v* после *expr* определяется:</span><span class="sxs-lookup"><span data-stu-id="3c9c4-433">The definite assignment state of *v* after *expr* is determined by:</span></span>
    * <span data-ttu-id="3c9c4-434">Если *expr_cond* является константным выражением ([константные выражения](expressions.md#constant-expressions)) со значением `true` состояние переменной *v* после *expr* совпадает со значением состояния *v* после *expr_true*.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-434">If *expr_cond* is a constant expression ([Constant expressions](expressions.md#constant-expressions)) with value `true` then the state of *v* after *expr* is the same as the state of *v* after *expr_true*.</span></span>
    * <span data-ttu-id="3c9c4-435">В противном случае, если *expr_cond* является константным выражением ([константные выражения](expressions.md#constant-expressions)) со значением `false` состояние переменной *v* после *expr* совпадает со значением состояния *v* после *expr_false*.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-435">Otherwise, if *expr_cond* is a constant expression ([Constant expressions](expressions.md#constant-expressions)) with value `false` then the state of *v* after *expr* is the same as the state of *v* after *expr_false*.</span></span>
    * <span data-ttu-id="3c9c4-436">В противном случае, если состояние *v* после *expr_true* присваивается определенным образом и состояние *v* после *expr_false* определенно назначена, состояние *v* после *expr* присваивается определенным образом.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-436">Otherwise, if the state of *v* after *expr_true* is definitely assigned and the state of *v* after *expr_false* is definitely assigned, then the state of *v* after *expr* is definitely assigned.</span></span>
    * <span data-ttu-id="3c9c4-437">В противном случае состояние *v* после *expr* определенно не назначается.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-437">Otherwise, the state of *v* after *expr* is not definitely assigned.</span></span>

#### <a name="anonymous-functions"></a><span data-ttu-id="3c9c4-438">Анонимные функции</span><span class="sxs-lookup"><span data-stu-id="3c9c4-438">Anonymous functions</span></span>

<span data-ttu-id="3c9c4-439">Для *lambda_expression* или *anonymous_method_expression* *expr* с текстом (либо *блок* или *выражение* ) *текст*:</span><span class="sxs-lookup"><span data-stu-id="3c9c4-439">For a *lambda_expression* or *anonymous_method_expression* *expr* with a body (either *block* or *expression*) *body*:</span></span>

*  <span data-ttu-id="3c9c4-440">Состояние определенного присваивания внешней переменной *v* перед *текст* совпадает со значением состояния *v* перед *expr*.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-440">The definite assignment state of an outer variable *v* before *body* is the same as the state of *v* before *expr*.</span></span> <span data-ttu-id="3c9c4-441">То есть состояние определенного присваивания внешних переменных наследуется из контекста анонимной функции.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-441">That is, definite assignment state of outer variables is inherited from the context of the anonymous function.</span></span>
*  <span data-ttu-id="3c9c4-442">Состояние определенного присваивания внешней переменной *v* после *expr* совпадает со значением состояния *v* перед *expr*.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-442">The definite assignment state of an outer variable *v* after *expr* is the same as the state of *v* before *expr*.</span></span>

<span data-ttu-id="3c9c4-443">Пример</span><span class="sxs-lookup"><span data-stu-id="3c9c4-443">The example</span></span>
```csharp
...
```
<span data-ttu-id="3c9c4-444">вызывает ошибку времени компиляции с момента `max` не является определенно присвоенной где объявлен анонимной функции.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-444">generates a compile-time error since `max` is not definitely assigned where the anonymous function is declared.</span></span> <span data-ttu-id="3c9c4-445">Пример</span><span class="sxs-lookup"><span data-stu-id="3c9c4-445">The example</span></span>
```csharp
delegate void D();
...
```
<span data-ttu-id="3c9c4-446">также вызывает ошибку времени компиляции с момента назначения `n` в анонимной функции не влияет на состояние определенного присваивания `n` вне анонимной функции.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-446">also generates a compile-time error since the assignment to `n` in the anonymous function has no affect on the definite assignment state of `n` outside the anonymous function.</span></span>

## <a name="variable-references"></a><span data-ttu-id="3c9c4-447">Ссылки на переменные</span><span class="sxs-lookup"><span data-stu-id="3c9c4-447">Variable references</span></span>

<span data-ttu-id="3c9c4-448">Объект *variable_reference* — *выражение* , классифицируется как переменная.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-448">A *variable_reference* is an *expression* that is classified as a variable.</span></span> <span data-ttu-id="3c9c4-449">Объект *variable_reference* обозначает место хранения, может осуществляться как получить текущее значение и сохранить новое значение.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-449">A *variable_reference* denotes a storage location that can be accessed both to fetch the current value and to store a new value.</span></span>
...
<span data-ttu-id="3c9c4-450">В C и C++ *variable_reference* называется *lvalue*.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-450">In C and C++, a *variable_reference* is known as an *lvalue*.</span></span>

## <a name="atomicity-of-variable-references"></a><span data-ttu-id="3c9c4-451">Атомарность ссылок на переменные</span><span class="sxs-lookup"><span data-stu-id="3c9c4-451">Atomicity of variable references</span></span>

<span data-ttu-id="3c9c4-452">Операции чтения и записи из следующих типов данных являются атомарными: `bool`, `char`, `byte`, `sbyte`, `short`, `ushort`, `uint`, `int`, `float`и ссылочные типы.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-452">Reads and writes of the following data types are atomic: `bool`, `char`, `byte`, `sbyte`, `short`, `ushort`, `uint`, `int`, `float`, and reference types.</span></span> <span data-ttu-id="3c9c4-453">Кроме того чтение и запись типов перечисления с базовым типом, в списке выше также являются атомарными.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-453">In addition, reads and writes of enum types with an underlying type in the previous list are also atomic.</span></span> <span data-ttu-id="3c9c4-454">Выполняет чтение и запись других типов, включая `long`, `ulong`, `double`, и `decimal`, а также определяемые пользователем типы, не гарантируется атомарным.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-454">Reads and writes of other types, including `long`, `ulong`, `double`, and `decimal`, as well as user-defined types, are not guaranteed to be atomic.</span></span> <span data-ttu-id="3c9c4-455">Помимо функций библиотеки, предназначенных для этой цели нет никакой гарантии из атомарных чтения, изменения и записи, такие как в случае инкремента или декремента.</span><span class="sxs-lookup"><span data-stu-id="3c9c4-455">Aside from the library functions designed for that purpose, there is no guarantee of atomic read-modify-write, such as in the case of increment or decrement.</span></span>
