...
Простые типы отличаются от других типов структуры, в том, что они разрешены некоторые дополнительные операции.
...
*  Конечный набор ненулевые значения формы `s * m * 2^e`, где `s` равно 1 или -1, и `m` и `e` определяются определенного типа с плавающей запятой: для `float`, `0 < m < 2^24` и `-149 <= e <= 104`и для `double`, `0 < m < 2^53` и `1075 <= e <= 970`. Денормализованные числа с плавающей запятой, считаются допустимыми значениями ненулевое значение.
